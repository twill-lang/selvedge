...
**selvedge does not run yet.**

selvedge is written in twill, in `.tw` files, and it uses `mode systems`, the
...
there is no binary, no `selvedge` command, and every code block below describes
behaviour selvedge is written to have rather than behaviour it has.
...
selvedge does in the meantime. That list is the useful output of this repository
today.
